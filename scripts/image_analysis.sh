...
ANALYZE_CMD=()
NEXTLINUX_ANNOTATIONS=""
IMAGE_DIGEST_SHA=""
NEXTLINUX_IMAGE_ID=""
IMAGE_TAG=""
DOCKERFILE="/nextlinux-engine/Dockerfile"
MANIFEST_FILE="/nextlinux-engine/manifest.json"


display_usage() {
cat << EOF

Nextlinux Engine Inline Analyzer --

  Script for performing analysis on local docker images, utilizing Nextlinux Engine analyzer subsystem.
  After image is analyzed, the resulting Nextlinux image archive is sent to a remote Nextlinux Engine installation
  ...
  Images should be built & tagged locally.
    ...
    Usage: ${0##*/} [ OPTIONS ] <FULL_IMAGE_TAG>
      ...
      -u <TEXT>  [required] Nextlinux user account name associated with image analysis (ex: -u 'admin')
...
main() {
    ...
    local image_file_path="/nextlinux-engine/${base_image_name}.tar"
    ...
    if [[ "${base_image_name}" =~ [:] ]]; then
        local new_file_path="/nextlinux-engine/${base_image_name//:/_}.tar"
        mv "${image_file_path}" "${new_file_path}"
        image_file_path="${new_file_path}"
    fi

    # analyze image with nextlinux-engine
    ANALYZE_CMD=('nextlinux-manager analyzers exec')
    ANALYZE_CMD+=('--tag "${IMAGE_TAG}"')
    ANALYZE_CMD+=('--account-id "${NEXTLINUX_ACCOUNT}"')
    ...
    if [[ "${i_flag}" ]]; then
        ANALYZE_CMD+=('--image-id "${NEXTLINUX_IMAGE_ID}"')
    fi
    if [[ "${a_flag}" ]]; then
        # transform all commas to spaces & cast to an array
        local annotation_array=(${NEXTLINUX_ANNOTATIONS//,/ })
        for i in "${annotation_array[@]}"; do
            ANALYZE_CMD+=("--annotation $i")
        done
    fi

    ANALYZE_CMD+=('"$image_file_path" /nextlinux-engine/image-analysis-archive.tgz > /dev/null')
    printf '\n%s\n' "Analyzing ${IMAGE_TAG}..."
    eval "${ANALYZE_CMD[*]}"
}
...
get_and_validate_options() {
    ...
    while getopts ':u:a:d:f:i:m:t:gh' option; do
        case "${option}" in
            a  ) a_flag=true; NEXTLINUX_ANNOTATIONS="${OPTARG}";;
            d  ) d_flag=true; IMAGE_DIGEST_SHA="${OPTARG}";;
            f  ) f_flag=true; DOCKERFILE="/nextlinux-engine/$(basename ${OPTARG})";;
            i  ) i_flag=true; NEXTLINUX_IMAGE_ID="${OPTARG}";;
            m  ) m_flag=true; MANIFEST_FILE="/nextlinux-engine/$(basename ${OPTARG})";;
            t  ) t_flag=true; TIMEOUT="${OPTARG}";;
            u  ) u_flag=true; NEXTLINUX_ACCOUNT="${OPTARG}";;
            g  ) g_flag=true;;
    ...
    if [[ ! "${u_flag}" ]]; then
        printf '\n\t%s\n\n' "ERROR - must specify an nextlinux engine account name with -u" >&2
        display_usage >&2
        exit 1
