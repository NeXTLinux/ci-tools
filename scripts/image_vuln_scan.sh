...
FILE_NAME=""
IMAGE_NAME=""
POLICY_BUNDLE="/nextlinux-engine/policy_bundle.json"
DOCKERFILE="/nextlinux-engine/Dockerfile"


display_usage() {
cat << EOF

Nextlinux Engine Inline Scan --

  Docker entrypoint for performing vulnerability analysis on local docker images.

  Starts Nextlinux Engine, Postgresql 9.6, and Docker Registry. 
  Finds docker image archives copied or mounted to /nextlinux-engine in the form of image+tag.tar.
  ...
  Usage: ${0##*/} [ -f ] [ -r ] [ -d Dockerfile ] [ -b policy.json ] [ -i IMAGE_ONE ]
      ...
      -d  [optional] Dockerfile name - must be mounted/copied to /nextlinux-engine.
      -i  [optional] Image name or file name location (use image name if piping in docker save stdout).
      -b  [optional] Nextlinux policy bundle name - must be mounted/copied to /nextlinux-engine.
...
main() {
    ...
    if [[ "${FILE_NAME}" ]]; then
        ...
        printf '%s\n\n' "Searching for Docker archive files in /nextlinux-engine."
        for i in $(find /nextlinux-engine -type f); do
            local file_name="$i"
            if [[ "${file_name}" =~ [:] ]]; then
    ...
    if [[ "${b_flag}" ]]; then
        (nextlinux-cli --json policy add "${POLICY_BUNDLE}" | jq '.policyId' | xargs nextlinux-cli policy activate) || \
    ...
    if [[ "${#FINISHED_IMAGES[@]}" -ge 1 ]]; then
        if [[ "${r_flag}" ]]; then
            for image in "${FINISHED_IMAGES[@]}"; do
                nextlinux_ci_tools.py -r --image "${image}"
            done
        fi
        echo
        for image in "${FINISHED_IMAGES[@]}"; do
            ...
            printf '%s\n\n' "-----------------------------------------------------------"
            (set +o pipefail; nextlinux-cli evaluate check "${image}" --detail | tee /dev/null; set -o pipefail)
        done

        if [[ "${f_flag}" ]]; then
            for image in "${FINISHED_IMAGES[@]}"; do
                nextlinux-cli evaluate check "${image}"
            done
        fi
    fi
}
...
get_and_validate_options() {
    ...
    while getopts ':d:b:i:fhr' option; do
        case "${option}" in
            d  ) d_flag=true; DOCKERFILE="/nextlinux-engine/$(basename $OPTARG)";;
            b  ) b_flag=true; POLICY_BUNDLE="/nextlinux-engine/$(basename $OPTARG)";;
    ...
    if [[ "${i_flag}" ]]; then
        if [[ -f "/nextlinux-engine/${IMAGE_NAME##*/}" ]] && [[ "${IMAGE_NAME}" =~ [.]tar? ]]; then
            FILE_NAME="/nextlinux-engine/${IMAGE_NAME##*/}"
        elif [[ "${IMAGE_NAME}" =~ (.*/|)([a-zA-Z0-9_.-]+)[:]([a-zA-Z0-9_.-]*) ]]; then
            FILE_NAME="/nextlinux-engine/${IMAGE_NAME##*/}.tar"
            if [[ ! -f "${FILE_NAME}" ]]; then
                cat <&0 > "${FILE_NAME}"
            fi
...
prepare_image() {
    ...
    local nextlinux_image_name="${NEXTLINUX_ENDPOINT_HOSTNAME}:5000/${image_repo}:${image_tag}"    
    ...
    skopeo copy --dest-tls-verify=false "docker-archive:${file}" "docker://${nextlinux_image_name}" &
    wait_proc="$!"
    wait "${wait_proc}"
    printf '\n%s\n' "Image archive loaded into Nextlinux Engine using tag -- ${nextlinux_image_name#*/}"

    start_scan "${file}" "${nextlinux_image_name}"
}

start_scan() {
    local file="$1"
    local nextlinux_image_name="$2"
    local wait_proc=""

    if [[ "${d_flag}" ]] && [[ -f "${DOCKERFILE}" ]]; then
        nextlinux-cli image add "${nextlinux_image_name}" --dockerfile "${DOCKERFILE}" > /dev/null
    else
        nextlinux-cli image add "${nextlinux_image_name}" > /dev/null
    fi

    # pass to background process & wait, required to handle keyboard interrupt when running container non-interactively.
    nextlinux_ci_tools.py --wait --timeout "${TIMEOUT}" --image "${nextlinux_image_name}" &
    wait_proc="$!"
    wait "${wait_proc}"

    FINISHED_IMAGES+=("${nextlinux_image_name}")
}
