...
display_usage() {
    ...
    Usage: ${0##*/} [ -s ] [ build | test | ci | function_name ] < build_version >
        ...
        build - Build image tagged IMAGE_REPO:dev using specified version of nextlinux-engine & engine-db-preload
...
PROJECT_VARS=( \
    "IMAGE_REPO=${IMAGE_REPO:=nextlinux/inline-scan}" \
...
main() {
    ...
    # Use the 'ci' param to execute a fully mocked CircleCI pipeline, utilizing docker in docker
        ...
        setup_build_environment
        ci_test_job 'docker.io/nextlinux/test-infra:latest' 'build_and_save_images'
        ci_test_job 'docker.io/nextlinux/test-infra:latest' 'test_built_images'
        ci_test_job 'docker.io/nextlinux/test-infra:latest' 'load_image_and_push_dockerhub'
    else
...
test_built_images() {
    ...
    if [[ "${build_version}" == 'all' ]]; then
        for version in "${BUILD_VERSIONS[@]}"; do
            unset NEXTLINUX_CI_IMAGE
            load_image "${version}"
            export NEXTLINUX_CI_IMAGE="${IMAGE_REPO}:dev-${version}"
            git reset --hard
            git checkout "tags/${version}" || { if [[ "$CI" == 'false' ]]; then true && local no_tag=true; else exit 1; fi; };
            if [[ "${CI}" == 'true' ]]; then
                test_inline_script "https://raw.githubusercontent.com/nextlinux/ci-tools/${version}/scripts/inline_scan"
            else
        ...
        export NEXTLINUX_CI_IMAGE="${IMAGE_REPO}:dev"
        if [[ "${build_version}" == 'latest' || "${build_version}" == 'dev' ]] && [[ "${CI}" == 'true' ]] ; then
            test_inline_script "https://raw.githubusercontent.com/nextlinux/ci-tools/${GIT_BRANCH}/scripts/inline_scan"
        elif [[ "${build_version}" == 'latest' || "${build_version}" == 'dev' ]] && [[ "${CI}" == 'false' ]] ; then
            test_inline_script "${WORKING_DIRECTORY}/scripts/inline_scan"
        else
            test_inline_script "https://raw.githubusercontent.com/nextlinux/ci-tools/${build_version}/scripts/inline_scan"
        fi
    fi
}
...
build_image() {
    ...
    echo "nextlinux_version=${nextlinux_version}"
    ...
    if [[ -z "${DB_PRELOAD_IMAGE}" ]]; then
        if [[ "${SLIM_BUILD}" == 'true' ]]; then
            local db_image="nextlinux/engine-db-preload-slim:${nextlinux_version}"
        else
            local db_image="nextlinux/engine-db-preload:${nextlinux_version}"
        fi
    else
        local db_image="${DB_PRELOAD_IMAGE}"
    fi
    if ! docker pull "${db_image}" &>/dev/null && ! docker inspect "${db_image}" &>/dev/null; then
        db_image="nextlinux/engine-db-preload:latest"
        docker pull "${db_image}"
    fi

    echo "Copying nextlinux-bootstrap.sql.gz from ${db_image} image..."
    db_preload_id=$(docker run -d --entrypoint tail "${db_image}" /dev/null | tail -n1)
    docker cp "${db_preload_id}:/docker-entrypoint-initdb.d/nextlinux-bootstrap.sql.gz" "${WORKING_DIRECTORY}/nextlinux-bootstrap.sql.gz"
    DOCKER_RUN_IDS+=("${db_preload_id:0:6}")

    if [[ "${nextlinux_version}" == "dev" ]]; then
        docker build --build-arg "NEXTLINUX_REPO=nextlinux/nextlinux-engine-dev" --build-arg "NEXTLINUX_VERSION=latest" -t "${IMAGE_REPO}:dev" .
    elif [[ "${nextlinux_version}" == "issue-712" ]]; then
        docker build --build-arg "NEXTLINUX_REPO=nextlinux/nextlinux-engine-dev" --build-arg "NEXTLINUX_VERSION=issue-712" -t "${IMAGE_REPO}:dev" .
    else
        docker build --build-arg "NEXTLINUX_VERSION=${nextlinux_version}" -t "${IMAGE_REPO}:dev" .
    fi
    local docker_name
    docker_name="${RANDOM:-temp}-db-preload"
    docker run -it --name "${docker_name}" "${IMAGE_REPO}:dev" debug /bin/bash -c "nextlinux-cli system wait --feedsready 'vulnerabilities' && nextlinux-cli system status && nextlinux-cli system feeds list"
    local docker_id
    docker_id=$(docker inspect ${docker_name} | jq '.[].Id')
    docker kill "${docker_id}" && docker rm "${docker_id}"
    DOCKER_RUN_IDS+=("${docker_id:0:6}")
    rm -f "${WORKING_DIRECTORY}/nextlinux-bootstrap.sql.gz"

    docker tag  "${IMAGE_REPO}:dev" "${IMAGE_REPO}:dev-${nextlinux_version}"
}
...
test_inline_script() {
    ...
    ${exec_cmd} -s "${INLINE_URL}" | bash -s -- -p  -t 1500 -b ".circleci/.nextlinux/policy_bundle.json" "nextlinux/nextlinux-engine:latest"
    ...
    popd
    ${exec_cmd} -s "${INLINE_URL}" | bash -s -- -t 900 -d ".circleci/node_critical_pass/Dockerfile" -b ".circleci/.nextlinux/policy_bundle.json" example.com:5000/ci-test_1/node_critical-pass
}
...
load_image() {
    local nextlinux_version="$1"
    docker load -i "${WORKSPACE}/caches/${PROJECT_REPONAME}-${nextlinux_version}-dev.tar"
    docker tag ${IMAGE_REPO}:dev ${IMAGE_REPO}:dev-${nextlinux_version}
}
...
push_dockerhub() {
    ...
    if [[ "${GIT_BRANCH}" == 'master' || "${CIRCLE_TAG:=false}" =~ ^v[0-9]+(\.[0-9]+)*$ ]] && [[ "${CI}" == true ]] && [[ ! "${nextlinux_version}" == 'latest' ]]; then
...
save_image() {
    ...
    docker save -o "${WORKSPACE}/caches/${PROJECT_REPONAME}-${nextlinux_version}-dev.tar" "${IMAGE_REPO}:dev"
}
