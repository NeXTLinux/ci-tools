...
ALL_REPORT_COMMANDS = {
    'content': 'nextlinux-cli --json image content',
    'vuln': 'nextlinux-cli --json image vuln',
    'details': 'nextlinux-cli --json image get',
    'policy': 'nextlinux-cli --json evaluate check'
}
...
def setup_parser():
    ...
    parser = argparse.ArgumentParser(description="A tool that automates various nextlinux engine functions for CI pipelines. Intended to be run directly on the nextlinux/nextlinux-engine container.")
    parser.add_argument('-a', '--analyze', action='store_true', help="Specify if you want image to be analyzed by nextlinux engine.")
    parser.add_argument('-r', '--report', action='store_true', help="Generate reports on analyzed image.")
    parser.add_argument('-s', '--setup', action='store_true', help="Sets up & starts nextlinux engine on running container.")
    parser.add_argument('-w','--wait', action='store_true', help="Wait for nextlinux engine to start up.")
...
def add_image(image_name):
    ...
    print ("\nImage submitted to Nextlinux Engine: {}".format(image_basename), flush=True)
    cmd = 'nextlinux-cli --json image add {}'.format(image_name).split()

    try:
        output = subprocess.check_output(cmd, stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as error:
        output = error.output
        raise Exception ("Failed to add image to nextlinux engine. Error: {}".format(output.decode('utf-8')))
...
def generate_reports(image_name, content_type=['all'], report_type=['all'], vuln_type='all', report_directory="nextlinux-reports"):
    image_basename = re.match(r'(?:.+\/)?([^:+].+)', image_name).group(1)
...
def get_config(config_path='/config/config.yaml', config_url='https://raw.githubusercontent.com/nextlinux/ci-tools/master/conf/stateless_ci_config.yaml'):
...
def get_image_digest(img_name, user='admin', pw='foobar', engine_url='http://localhost:8228/v1/images'):
    cmd = 'nextlinux-cli --json image get {}'.format(img_name).split()
...
def get_image_info(img_digest, user='admin', pw='foobar', engine_url='http://localhost:8228/v1/'):
    ...
    if r.status_code == 200:
        ...
        raise Exception ("Bad response from Nextlinux Engine - httpcode={} data={}".format(r.status_code, r.text))
...
def is_engine_running():
    ...
    if 'nextlinux-manager' in output or 'twistd' in output:
        return True
    else:
        return False
...
def start_nextlinux_engine():
    if not is_engine_running():
        cmd = 'nextlinux-manager service start --all'.split()
        print ("Starting nextlinux engine...", flush=True)
        try:
            with open('nextlinux-engine.log', 'w') as out:
                subprocess.Popen(cmd, stdout=out, stderr=subprocess.STDOUT)
        except Exception as error:
            raise Exception ("Unable to start nextlinux engine. Exception: {}".format(error))

        return True
    else:
        raise Exception ("Nextlinux engine is already running.")
...
def main(arg_parser):
    ...
    # set default nextlinux cli environment variables if they aren't already set
    for var in nextlinux_env_vars.keys():
        if var not in os.environ:
            os.environ[var] = nextlinux_env_vars[var]

    nextlinux_user = os.environ['NEXTLINUX_CLI_USER']
    nextlinux_pw = os.environ['NEXTLINUX_CLI_PASS']
    nextlinux_cli_url = os.environ['NEXTLINUX_CLI_URL']
    feeds_url='{}/system/feeds'.format(re.sub('[/]$', '', nextlinux_cli_url))
    health_url='{}/health'.format(re.sub('/v1[/]*$', '', nextlinux_cli_url))

    if wait_engine:
        if image_name:
            img_digest = get_image_digest(image_name, user=nextlinux_user, pw=nextlinux_pw)
            wait_image_analyzed(img_digest, timeout=timeout, user=nextlinux_user, pw=nextlinux_pw, engine_url=nextlinux_cli_url)
        else:
            wait_engine_available(health_check_urls=[health_url, feeds_url], timeout=timeout, user=nextlinux_user, pw=nextlinux_pw)

    elif setup_engine:
        get_config()
        start_nextlinux_engine()
        wait_engine_available(health_check_urls=[health_url, feeds_url], timeout=timeout, user=nextlinux_user, pw=nextlinux_pw)
